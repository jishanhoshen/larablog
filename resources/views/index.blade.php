...
<div class="container mx-auto px-5">
    <div class="px-10 py-5 flex">
        <div class="w-2/3">
            <div class="toppostslide owl-carousel owl-theme relative px-6 inline rounded ">
                ...
                <div class="h-96 relative">
                    <img src="{{ asset('image/'.$post->image) }}" alt="{{ $post->title }}s" class="h-full w-full rounded object-cover object-center" />
                    ...
                    <div class="posttitle absolute bottom-6 inset-x-0 mx-6 text-slate-50">
                        <div class="mb-2">
                            <a href="" class="capitalize">{{ $post->category }}</a>
                            /
                            <a href="" class="capitalize">{{ date('F j Y', strtotime($post->date)) }}</a>
                        </div>
        ...
        <div class="w-1/3 px-4 py-2 rounded">
            ...
            <div class="h-16 bg-slate-50 mt-4 relative">
                <img src="{{ asset('image/'.$category->image) }}" alt="" class="w-full h-full object-cover object-center">
                ...
                <h2 class="absolute inset-0 flex justify-center items-center text-slate-50 text-xl font-bold">
                    <a href="" class="capitalize">{{ $category->name }}</a>
                </h2>
            </div>
            @endforeach
    ...
    <div class="px-10 py-5 flex flex-wrap justify-center gap-3 ">
        ...
        <div class="card w-80 bg-base-100 shadow-xl mx-4 my-2 inline ">
            <figure class="h-48 overflow-hidden"><img src="{{ asset('image/'.$post->image) }}" alt="{{ $post->title }}" class="object-cover object-center h-full w-full" /></figure>
            <div class="card-body relative">
                <div class="badge badge-secondary absolute right-4 top-3">NEW</div>
                <h2 class="card-title inline-block overflow-hidden text-ellipsis whitespace-nowrap">
                    {{ $post->title }}
                </h2>
    ...
    <div class="px-10 py-5">
        <div class="featured owl-carousel owl-theme relative px-6 inline rounded ">
            @foreach($posts as $post)
            <div class="h-96 relative">
                <img src="{{ asset('image/'.$post->image) }}" alt="{{ $post->title }}" class="h-full w-full rounded object-cover object-center" />
                <div class="bg-slate-900 h-full w-full absolute top-0 opacity-50 rounded"></div>
                <div class="posttitle absolute bottom-6 inset-x-0 mx-6 text-slate-50">
                    <div class="mb-2">
                        <a href="" class="capitalize">{{ $post->category }}</a>
    ...
    <div class="px-16 py-5 flex justify-between">
        <div class="w-8/12">
            ...
            <div class="flex juslify-between">
                @foreach($posts as $post)
                <div class="mt-4 w-[calc(50%-1rem)] first:mr-4 last:ml-4">
                    <div class="bg-slate-50">
                        <img src="{{ asset('image/'.$post->image) }}" alt="{{ $post->title }}" class="h-52 w-full object-cover object-center">
                        <div class="p-6">
                            <p class="text-sm pb-1"><a href="" class="text-red-600">Category</a> / <a href="" class="text-slate-600">Post Date</a></p>
                            <h1 class="text-2xl text-slate-900 font-semibold overflow-hidden text-ellipsis whitespace-nowrap pb-1"><a href="">{{ $post->title }}</a> </h1>
